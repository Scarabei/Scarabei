...
public class RedTransformOLD implements CanvasTransform {
	...
	private void setup () {
		...
		MathTools.setupScaleMatrix(scale_matrix, this.getScaleX(), this.getScaleY(), 1d);
		if (log) scale_matrix.print("scale_matrix");
		MathTools.setupRotationMatrix(rotation_matrix, this.position.getRotation().toRadians());
		if (log) rotation_matrix.print("rotation_matrix");
		MathTools.setupOffsetMatrix(offset_matrix, this.position.getX(), this.position.getY());
		if (log) offset_matrix.print("offset_matrix");
		MathTools.setupSkewMatrix(skew_matrix, this.getSkewX(), this.getSkewY());
		if (log) skew_matrix.print("skew_matrix");

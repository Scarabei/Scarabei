...
public class RedTriangle extends VertexMaster implements Triangle {
	...
	return (MathTools.pointLiesInsideTriangle(tmp.getX(), tmp.getY(), A.transformed().getX(),
		A.transformed().getY(), B.transformed().getX(), B.transformed().getY(), C.transformed().getX(),
		C.transformed().getY()));
    }
}

...
public class RemoteFile implements File {
	...
	public RemoteFile(AbsolutePath<FileSystem> output_file_path,
	...
	public boolean rename(String new_name) {
		java.io.File f = new java.io.File(this.getAbsoluteWindowsPathString());
		RemoteFile new_file = new RemoteFile(this.absolute_path.parent().child(
				new_name), this.fs);
	...
	public String getAbsoluteWindowsPathString() {
		...
		if (relative.length() > 0) {
			relative = RemoteFileSystem.OS_SEPARATOR + relative;
		}
	...
	public File child(String child_name) {
		return new RemoteFile(this.getAbsoluteFilePath().child(child_name),
				this.getFileSystem());
	}
	...
	public File parent() {
		if (!this.absolute_path.isRoot()) {
			return new RemoteFile(this.absolute_path.parent(), this.fs);
		}

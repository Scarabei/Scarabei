...
public class Geometry {
	...
	public static final void installComponent (GeometryComponent component_to_install) {
	...
	public static boolean equalFloat2Collections (Collection<? extends FixedFloat2> A, Collection<? extends FixedFloat2> B) {
		return invoke().equalPointCollections(A, B);
	}

	public static <T extends FixedFloat2> void setPointsCollectionSize (EditableCollection<T> A, int size) {
		invoke().setPointsCollectionSize(A, size);
	}

	public static void copyValues (Collection<? extends FixedFloat2> A, EditableCollection<? extends Float2> B) {
		invoke().copyValues(A, B);
	}

	public static void copyValues (Collection<? extends FixedFloat2> A, EditableCollection<? extends Float2> B, int offset) {
		invoke().copyValues(A, B, offset);
	}

	public static void applyTransformation (Matrix transformation, EditableCollection<? extends Float2> points_to_process) {
	...
	public static void parametrize (FixedFloat2 A, double progress_from_A_to_B, FixedFloat2 B, Float2 result) {
		invoke().parametrize(A, progress_from_A_to_B, B, result);
	}

	public static void parametrize (CanvasPosition A, double progress_from_A_to_B, CanvasPosition B, CanvasPosition result) {
		invoke().parametrize(A, progress_from_A_to_B, B, result);
	}
	...
	public static Rectangle setupWrapingFrame (Collection<? extends FixedFloat2> points_to_wrap, Rectangle wrapping_frame) {
		return invoke().setupWrapingFrame(points_to_wrap, wrapping_frame);
	}

	public static <T extends EditableCollection<Float3>> T newFloat3 (T target_list, int how_many_to_add) {
		return invoke().newFloat3(target_list, how_many_to_add);
	}

	public static <T extends EditableCollection<Float2>> T newFloat2 (T target_list, int how_many_to_add) {
		return invoke().newFloat2(target_list, how_many_to_add);
	}

	public static ClosedPolygonalChain newClosedPolygonalChain (Collection<Float2> vertices) {
	...
	public static void fillUpFloat2 (EditableCollection<Float2> output, int desired_size) {
		invoke().fillUpFloat2(output, desired_size);
	}

	public static void fillUpFloat3 (EditableCollection<Float3> output, int desired_size) {
